...
export default function ApplicationsPage() {
  ...
  const loadApplications = async (uid: string) => {
    try {
      const { data:apps } = await supabase.from('applications').select('*').eq('candidate_id',uid).order('applied_at',{ ascending:false })
      ...
      const enriched = await Promise.all(apps.map(async (app:any) => {
        ...
        const { data:company } = job ? await supabase.from('profiles').select('name').eq('id',job.company_id).single() : { data:null }
  ...
  const counts = { all:applications.length, applied:applications.filter(a=>a.status==='applied').length, shortlisted:applications.filter(a=>a.status==='shortlisted').length, rejected:applications.filter(a=>a.status==='rejected').length }
    ...
    <>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'2rem 1.5rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10, marginBottom:'1.5rem' }}>
          {[
            { label:'Total', count:counts.all, color:'#818cf8', bg:'rgba(99,102,241,0.12)' },
            { label:'Applied', count:counts.applied, color:'#818cf8', bg:'rgba(99,102,241,0.08)' },
            { label:'Shortlisted', count:counts.shortlisted, color:'#34d399', bg:'rgba(16,185,129,0.1)' },
            { label:'Rejected', count:counts.rejected, color:'#f87171', bg:'rgba(239,68,68,0.1)' },
          ].map(s => (
        ...
        {loading ? (
          ...
          <div style={{ textAlign:'center', padding:'5rem', color:'#6b7280' }}>
            ...
            <p style={{ fontSize:16, fontWeight:700, color:'#e5e7eb', marginBottom:6 }}>{filter==='all'?'No applications yet':`No ${filter} applications`}</p>
            <p style={{ fontSize:13, marginBottom:'1.25rem' }}>{filter==='all'?'Start applying to jobs to track them here':`You have no ${filter} applications`}</p>
            {filter==='all'&&(
              <a href="/jobs" style={{ textDecoration:'none' }}>
                <button style={{ padding:'9px 20px', background:'#6366f1', border:'none', borderRadius:8, color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', fontFamily:'Inter,sans-serif' }}>Browse jobs →</button>
              </a>
            )}
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {filtered.map((app:any) => {
              ...
              return (
                <div key={app.id} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:16, padding:'1.25rem', display:'flex', alignItems:'flex-start', gap:14, transition:'all 0.2s' }}
                ...
                >
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:4 }}>
                      <span style={{ fontWeight:700, fontSize:15, color:'#f1f1f1', letterSpacing:'-0.2px' }}>{app.job?.title||'Job no longer available'}</span>
                  ...
                  {app.job&&app.status!=='rejected'&&(
                    <button onClick={()=>startChat(app.job.company_id,app.job_id)} disabled={startingChat===app.job.company_id} style={{ display:'flex', alignItems:'center', gap:6, padding:'7px 14px', background:'#6366f1', border:'none', borderRadius:8, color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'Inter,sans-serif', flexShrink:0 }}>
                      {startingChat===app.job.company_id?<Loader2 size={12} style={{ animation:'spin 1s linear infinite' }}/>:<MessageSquare size={12}/>}
                      Message
                    </button>
                  )}
                </div>
