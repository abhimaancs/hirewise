...
const companies = [
  ...
  { name:'CRED', industry:'Fintech', location:'Bangalore', size:'500–1,000', jobs:5, color:'#111827', desc:'Members-only credit card bill payment platform with exclusive rewards.' },
  ...
  { name:'Chargebee', industry:'SaaS', location:'Chennai', size:'500–1,000', jobs:3, color:'#6366f1', desc:'Subscription billing and revenue management for SaaS companies globally.' },
...
const industries = ['All','Fintech','Dev Tools','SaaS','E-Commerce','Food Tech','Quick Commerce','Payments']
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .company-card{transition:all .25s!important}
      ...
      <div style={{maxWidth:1100,margin:'0 auto',padding:'3rem 2rem',position:'relative',zIndex:1}}>
        ...
        <div className="companies-grid fade-up-2" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
          {companies.map(co => (
            <div key={co.name} className="company-card" style={{background:'rgba(255,255,255,.02)',border:'1px solid rgba(255,255,255,.07)',borderRadius:18,padding:'1.5rem',cursor:'default'}}>
              ...
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                ...
                <Link href="/signup">
                  <button style={{padding:'6px 14px',background:'rgba(99,102,241,.15)',border:'1px solid rgba(99,102,241,.25)',borderRadius:8,color:'#818cf8',fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif',transition:'all .2s'}}
                    onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background='#6366f1';(e.currentTarget as HTMLElement).style.color='#fff'}}
                    onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background='rgba(99,102,241,.15)';(e.currentTarget as HTMLElement).style.color='#818cf8'}}>
                    View roles →
                  </button>
                </Link>
              </div>
